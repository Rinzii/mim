...
// https://clang.llvm.org/docs/UsersManual.html#controlling-diagnostics-via-pragmas
...
#ifndef MIM_ALIGNAS
    #if defined(MIM_COMPILER_NO_ALIGNAS)
        #define MIM_ALIGNAS(alignment)
    #elif defined(MIM_COMPILER_MSVC)
        #define MIM_ALIGNAS(alignment) __declspec(align(alignment))
    #elif defined(MIM_COMPILER_CLANG)
        #define MIM_ALIGNAS(alignment) alignas(alignment)
    #elif defined(MIM_COMPILER_GCC)
        #define MIM_ALIGNAS(alignment) __attribute__((aligned(alignment)))
    #else
        #define MIM_ALIGNAS(alignment) alignas(alignment)
    #endif
#endif
...
#ifndef MIM_ALIGNOF
    #if !defined(MIM_COMPILER_NO_ALIGNOF) || MIM_COMPILER_NO_ALIGNOF == 0
        #define MIM_ALIGNOF(type) alignof(type)
    #else
        #define MIM_ALIGNOF(type) MIM_STATIC_ASSERT(sizeof(type) == sizeof(type))
        MIM_WARNING("MIM_ALIGNOF is not supported on this compiler")
    #endif
#endif
...
#ifndef MIM_CONSTEXPR
    #ifndef MIM_COMPILER_NO_CONSTEXPR
        #define MIM_CONSTEXPR constexpr
    #else
        #define MIM_CONSTEXPR
    #endif
#endif
        ...
        #define MIM_NODISCARD [[nodiscard]]
            ...
            #include <sal.h>
            #define MIM_NODISCARD __checkReturn // Check that GNUC is at least 7.1
    ...
    #define MIM_INLINE inline
            ...
            #endif
        #endif
    #else // If we are in debug mode, disable force inline to allow easier debugging.
        #if defined(__cplusplus)
            #define MIM_FORCE_INLINE inline
        #else
            #define MIM_FORCE_INLINE __inline
...
#ifndef MIM_RESTRICT
    #if defined(MIM_COMPILER_MSVC)
        #define MIM_RESTRICT __restrict
    #elif defined(MIM_COMPILER_GCC) || defined(MIM_COMPILER_CLANG)
        #define MIM_RESTRICT __restrict__
    #else
        #define MIM_RESTRICT
    #endif
#endif
...
#ifndef MIM_NOEXCEPT
    #if defined(MIM_COMPILER_CPP11_ENABLED)
        #define MIM_NOEXCEPT noexcept
        #define MIM_NOEXCEPT_EXPR(x) noexcept(x)
    #else
        #define MIM_NOEXCEPT
        #define MIM_NOEXCEPT_EXPR(x)
    #endif
#endif
...
#define MIM_DISABLE_CLANG_WARNING(w)
        ...
        #define MIM_RESTORE_CLANG_WARNING()    \
                    MIM_PRAGMA("clang diagnostic pop")
    #else
        #define MIM_RESTORE_CLANG_WARNING()
    #endif
#endif
